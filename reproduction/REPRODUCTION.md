...
Terminology, used consistently below: a learned **mixture** is K crafted
...
*training* = the learning loop that produces a "checkpoint" (K system prompts
...
- (a) re-ran the *scoring math* on the authors' saved inference outputs (the
  simulated game answers their learned mixtures generated, shipped in
...
home), but the system prompt texts themselves remain usable — re-learning *only the
weights* against the new backbone recovers most of the alignment (**0.57**),
and re-learning the whole mixture recovers all of it (0.31). The mis-calibration is also
...
Every learned system prompt gives one exact answer 100% of the time (survives
...
- *Step 1 — test the paper's 1D-learned mixture on a second game:* took the system
  prompts learned on Public Goods only; asked each one both questions; generated
...
- *Step 4 — check what learning both games together costs in single-game accuracy:*
  score each game's histogram from the two-game mixture by itself. **Result:
  somewhat less accurate than learning that game alone** (Public Goods W-dist
...
The learned artifact ("checkpoint") is the set of system prompts + weights.
...
re-learned from 10 fresh samples per prompt), then 1,000 freshly generated
evaluation samples:
...
| Metric | From-scratch mixture | Table 2 | Human floor (1,000 samples) |
|---|---|---|---|
...
The independently learned mixture slightly outperforms the published number
...
learned on GPT-4o in finding 1(c), and the reweighting row uses the authors'
...
| Authors' prompts + weights re-learned from 10 flash samples each, on flash | **0.57** | 9.89 / 6.40 | pass |
| human | — | 9.63 / 6.44 | — |
...
from 2.04–2.17 to 0.57; learning from scratch (finding 3) reaches 0.31 — weight
...
### From-scratch re-learning: the method self-corrects on every backbone — and the cheap model wins

Public Goods, EM K=10, all three backbones:

| Backbone | learned-mixture W-dist | mean/std (human 9.63/6.44) | Wilcoxon |
|---|---|---|---|
...
All the diversity in a Gemini mixture comes from the weights: the learning
...
A learned flash system prompt answers identically 10/10 even at temperature 2.0
...
The paper learns each game separately and mentions multi-attribute joint
learning only as future work. `data/joint.csv` records the same participant's
...
### Stage A — what the 1D-learned system prompts imply about the joint

Sample system prompts from a learned mixture; each plays both games (independent
...
softens this to +0.79. The failure is symmetric: learning on Dictator instead
...
![Same person, two games: 1D-learned system prompts fall on a line; joint-learned system prompts spread like real people](figures/2d_diagonal_vs_spread.png)
...
PG-learned flash mixture, correlation stays +1.0; 0.259 → 0.129 on the
Dictator-learned one, correlation still +0.44 vs the human +0.057 — better,
...
### Stage C — learning both games together fixes it
...
| PG x Dictator | learned on Public Goods only (the paper's method) | learned on both games, K=25 | learned on both games, K=100 | real people |
|---|---|---|---|---|
...
Reading the table: the column names say which data each mixture was *learned* from;
...
statistically indistinguishable from human; the K=100 mixture presses against the
...
mixture came in at ~96% of its theoretical optimum; details in the appendix).
...
The contrast between what 1D and 2D learning produces is visible in the prompt
texts themselves. Typical **1D-learned** system prompts describe globally
...
exactly why every answer-pair lands on the diagonal. The **2D-learned**
...
Full texts for every learned system prompt are in the artifact CSVs and
`fit_trace.json` files (see the appendix's artifact index).

### The trade-off: the two-game mixture is slightly less accurate on each single game

Method: take the K=100 two-game mixture's 1,000 generated answer-pairs and look
...
dedicated single-game mixtures:

| Game (W-dist vs full human data) | from the two-game mixture | dedicated single-game mixture | paper's GPT-4o |
|---|---|---|---|
| Public Goods | 0.47 | **0.31** (flash EM, K=10) | 0.47 |
| Dictator | 1.56 | **0.68** (flash EM, K=50) | 1.69 |

The two-game mixture's per-game histograms are ~1.5–2x less accurate than
the dedicated mixtures — while still matching the paper's published GPT-4o quality
on both games. Two causes: the 100 system prompts must serve both games at
once (a dedicated mixture puts all its resolution into one game), and the
two-game mixture was learned from the 2,520 participants who played *both* games
...
than learning error. Net: one mixture that gets the *combinations* right
versus separate mixtures that get each game sharpest and the combinations entirely
wrong.
...
from-scratch re-learning on an independent account (finding 1, finding 3).

Its learned artifacts are backbone-specific even though the method is not:
...
1D-learned mixtures imply joint behavior no human population has, which only
joint re-learning — not reweighting — repairs (finding 5). Distributional
